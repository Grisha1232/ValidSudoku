...
class SettingsModel {
    ...
    public static let textHowToPlay = """
        В большой квадрат выборочно внесены цифры в некоторые клетки. Смысл задания заключается в том, что необходимо заполнить недостающие пустые клетки. Правила достаточно просты, что исключает множественные решения.
        
        В каждой колонке или ряду должны быть проставлены цифры от 1 до 9 без повторений. Также в каждом из маленьком квадрате должны проставлены цифры от 1 до 9 без повторений. Судоку разделяются на несколько уровне сложности - в зависимости от количество заполненных цифр. Чем из меньше, тем сложнее игра.
